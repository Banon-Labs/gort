...
On a recognized `KLAATU BERADA NIKTO` trigger, perform repo grounding and controller bootstrap silently. Use the authoritative instruction file for the current Gort action as the bootstrap entrypoint. In a fresh context, if the current action is already anchored to an active repository/worktree that has a repo-root `AGENTS.md`, that repo-root `AGENTS.md` is authoritative and must be the grounding `read`; do **not** bypass it in favor of `~/projects/AGENTS.md` merely because the repo-local file was not previously known. Within `~/projects`, use `~/projects/AGENTS.md` only as the fallback grounding file when no more specific active-repo `AGENTS.md` is already available for the current action. If the active repo lacks a repo-root `AGENTS.md`, use the nearest parent authoritative `AGENTS.md` without speculative missing-file reads or user-visible ENOENT recovery chatter. After the grounding read, read `/home/choza/projects/gort/gort.md`. Do not emit visible trigger-interpretation text such as debating whether the recognized cue was intended.
